...
export class DevopsController {
    ...
    @ApiBody({
        description: 'Crea un mensaje, mediante un UsersDto. Devuelve retorna en formato json si se realiza con exito.',
        type: UsersDto,
        examples: {
            ejemplo1: {
                value: {
    ...
    createMessage(@Body() data: { message: string, to: string, from: string, timeToLifeSec: string }, @Res() res) {
        res.json(this.devopsService.createMessage(data.message, data.to, data.from, data.timeToLifeSec))
    }
    @Put()
    async findPut() {
